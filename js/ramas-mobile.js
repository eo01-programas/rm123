(() => {
    ...
    function filterByExactOpPartida(query) {
        ...
        if (!norm) return [];
        return state.records.filter(record => getRecordOpPartidaKey(record) === norm);
    }
    ...
    // Los campos solo se re-arman cuando cambia la seleccion, el tipo o el
    // estado de sus pases: un refresh en segundo plano re-renderiza los
    // resultados y no debe pisar lo que el operario escribe ni sacarlo del
    // modo edicion.
    function computeFormKey() {
        return `${getPrefix()}::` + Array.from(state.selectedIds).sort().map(id => {
    ...
    function showFormCard(selectedCount) {
        ...
        const formKey = computeFormKey();
        const selectionChanged = formKey !== state.lastFormKey;
        state.lastFormKey = formKey;

        if (!selectionChanged && state.editMode) {
        ...
        if (selectionChanged) clearFormFields();
    ...
    function exitEditMode() {
        state.editMode = false;
        state.editPassIndex = -1;
        // Forzar que showFormCard re-arme los campos con los datos actuales
        state.lastFormKey = null;
        const editModeActions = document.getElementById('ramas-edit-mode-actions');
        if (editModeActions) editModeActions.classList.add('hidden');
